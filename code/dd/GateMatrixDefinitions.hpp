...
// Single qubit gate matrices. The indices of values are ordered as [00, 01, 10, 11].
// Commonly used gates are Imat, Hmat, Xmat and so on.
...
// Two qubit matrix. The indices of values are ordered as [[0000, 0001, 0010, 0011], ...] where the first two are
// row indices, and the last two are column indices.
// Commonly used two qubit matrices are CXmat, CZmat and so on.
